...
export class LocationsMap implements OnInit {
  ...
  mapHeight: SafeStyle;
  @Output() locationSelect: EventEmitter<FOLocation> = new EventEmitter<
    FOLocation
  >();
  ...
  loadMap(): void {
    ...
    this.map = new google.maps.Map(
      document.querySelector("locations-map #map"),
      mapOptions
    );
    this.service
      .getCurrentPosition()
      .then(position => this.setCurrentPositionMarker(position));
    let infoWindow = new google.maps.InfoWindow();
    let bounds = new google.maps.LatLngBounds();
    for (let i = 0; i < this.locationList.length; i++) {
      let latLng = new google.maps.LatLng(
        this.locationList[i].latitude,
        this.locationList[i].longitude
      );
      ...
      let infoWindowContent =
        "<h6>" +
        this.locationList[i].address_section_1 +
        "</h6>" +
        "<h8>" +
        (this.locationList[i].is_open ? "OPEN" : "CLOSED") +
        " NOW</h8>";
  ...
  onNavigateClick(): void {
    ...
    window.open(
      "https://maps.google.com/?saddr=My+Location&daddr=" + latLng,
      "_system"
    );
  }
